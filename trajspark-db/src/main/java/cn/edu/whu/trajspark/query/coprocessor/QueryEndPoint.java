...
import static cn.edu.whu.trajspark.coding.utils.DateTimeParse.timeToZonedTime;
...
public class QueryEndPoint extends QueryService implements Coprocessor, CoprocessorService {
  ...
  protected Scan getScanWithColumns(QueryRequest request) {
    ...
    scan.addColumn(COLUMN_FAMILY, START_TIME_QUALIFIER);
    scan.addColumn(COLUMN_FAMILY, END_TIME_QUALIFIER);
    return scan;
  }
  ...
  protected boolean coarseFilter(Result result, QueryRequest queryRequest) {
    ...
    if (queryRequest.hasTemporalQueryWindows() & !queryRequest.hasSpatialQueryWindow()) {
      ...
      for (TemporalQueryWindow temporalQueryWindow : temporalQueryWindowList) {
        TimeLine timeLine = new TimeLine(timeToZonedTime(temporalQueryWindow.getStartMs()),
            timeToZonedTime(temporalQueryWindow.getEndMs()));
  ...
  protected boolean temporalFineFilter(Result result, QueryRequest queryRequest)
      throws IOException {
    TimeLine trajectoryTimeLine = getTrajectoryTimeLine(result);
