...
public class STQueryEndPoint extends QueryCondition.QueryService implements Coprocessor, CoprocessorService {
  ...
  protected Scan buildScan() {
    ...
    scan.addColumn(TrajectorySerdeUtils.COLUMN_FAMILY, TrajectorySerdeUtils.START_POINT_QUALIFIER);
    scan.addColumn(TrajectorySerdeUtils.COLUMN_FAMILY, TrajectorySerdeUtils.END_POINT_QUALIFIER);
  ...
  private Result getMainIndexedResult(Result result) throws IOException {
    return instance.getDataSet(IndexTable.extractDataSetName(env.getRegionInfo().getTable().getNameAsString()))
        .getCoreIndexTable()
        .get(new Get(result.getValue(TrajectorySerdeUtils.COLUMN_FAMILY, TrajectorySerdeUtils.PTR_QUALIFIER)));
  }
  ...
  private QueryCondition.TrajectoryResult buildTrajectoryResult(Result result){
    return QueryCondition.TrajectoryResult.newBuilder()
        ...
        .setTrajPointList(
            ByteString.copyFrom(TrajectorySerdeUtils.getByteArrayByQualifier(result, TrajectorySerdeUtils.TRAJ_POINTS_QUALIFIER)))
