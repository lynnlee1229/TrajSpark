...
//            .getCoreIndexTable();
    ...
    LOGGER.info("DataSet {} load finished, cost time: {}ms.", dataSetMeta.getDataSetName(),
  ...
  public void storePointBasedTrajectory(JavaRDD<Trajectory> trajectoryJavaRDD) throws Exception {
    ...
    long startLoadTime = System.currentTimeMillis();
    LOGGER.info(
        "Start storing BasePointTrajectory into location : " + this.storeConfig.getLocation());
    initDataSetTest(storeConfig.getDataSetMeta());
    IndexMeta coreIndexMeta = storeConfig.getDataSetMeta().getCoreIndexMeta();
  ...
  public void bulkLoadToMainIndexTable(JavaRDD<Trajectory> trajectoryJavaRDD,
                                       IndexMeta mainIndexMeta) throws Exception {
    ...
    RegionLocator locator =
        instance.getConnection().getRegionLocator(TableName.valueOf(mainTableName));
    JavaRDD<Put> putJavaRDD = trajectoryJavaRDD.map(
        trajectory -> TrajectoryDataMapper.mapTrajectoryToSingleRow(trajectory, mainIndexMeta));
